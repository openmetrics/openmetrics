      ...
      attr_sorted.each do |attr|
        ...
        case attr
          when 'actions'
            ...
            # run action should append data-id, have empty href and .run_test_plan to trigger js handlers; also disable turbolinks here
            if append_actions_cell && append_actions_cell[:run]
              ac += link_to(raw('<span class="glyphicon glyphicon-play-circle"></span> Run'), '#',
                            class: 'btn btn-success btn-xs run_test_plan', data: { id: item[:id]})
            end
